...
if ~isempty(EEG.event)
    ...
    for index = 1:EEG.trials
            EEG.event(index).mknum = index;
        end;
    if EEG.segmented == 1
        for index = 1:EEG.trials
            EEG.event(1).latency = [];
            EEG.event(index).latency = (index-1)*EEG.pnts+EEG.offset+1;
        end;
        EEG.event.latency = num2str(EEG.event.latency);
    end
...
%     
    ...
    for index = 1:length(e)
        ...
        fprintf(fid2, 'Mk%s=%s,%s,%s,%d,0,0\n', num2str(e(index).mknum),num2str(e(index).type), num2str(tmpcom), e(index).latency, tmpdur);
    end;
end
fclose(fid1);
fclose(fid2);
